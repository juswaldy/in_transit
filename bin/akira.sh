...
if [ $step == "get" ]; then
	...
	if [ -z $ffmpegcrop ]; then
		ffmpeg -i ${rootfolder}/${youtubevideo} -q:v 1 ${originalfolder}/%06d.jpg
	else
		ffmpeg -i ${rootfolder}/${youtubevideo} -filter:v "$ffmpegcrop" -q:v 1 ${originalfolder}/%06d.jpg
	fi
...
################################################################################
	...
	#rm x.mp4 final.mp4
	#ffmpeg -framerate $fps -i $finalfolder/%06d.jpg -c:v h264_nvenc $rootfolder/x.mp4
...
################################################################################
	...
	#ffmpeg -framerate $fps -i $withstyle/%06d.jpg -c:v h264_nvenc $rootfolder/x.mp4
