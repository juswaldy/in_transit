...
if [ $step == "get" ]; then
	...
	if [ -z $ffmpegcrop ]; then
		ffmpeg -i ${rootfolder}/${youtubevideo} -q:v $vquality ${originalfolder}/%05d.jpg
	else
		ffmpeg -i ${rootfolder}/${youtubevideo} -filter:v "$ffmpegcrop" -q:v $vquality ${originalfolder}/%05d.jpg
	fi
