...
################################################################################
# Explode video frames.
################################################################################
elif [ $step == "frames" ]; then
	# Get frames.
	youtubevideo=`ls *$id*.mp4`
	if [ -z $ffmpegcrop ]; then
		ffmpeg -i ${rootfolder}/${youtubevideo} -q:v 1 ${originalfolder}/%07d.jpg
	else
		ffmpeg -i ${rootfolder}/${youtubevideo} -filter:v "$ffmpegcrop" -q:v 1 ${originalfolder}/%07d.jpg
	fi
...
################################################################################
	...
	makeframes ${originalfolder} $blackimage
