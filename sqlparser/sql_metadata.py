...
def _normalize(token: sqlparse.sql.Token) -> str:
	...
	if (
		...
		if schema_notation_tokens == schema_notation_match:
			...
			if len(tables) > 0:
				...
				tables.append(table_name)
		elif _normalize(tokens[index - 1]) not in [",", last_keyword]:
...
def get_query_tables(query: str) -> List[str]:
	...
	for index, token in enumerate(tokens):
		...
		token_val_norm = _normalize(token)
...
def get_query_table_aliases(query: str) -> Dict[str, str]:
	...
	for t in tokens:
		...
		if last_keyword_token:
			last_keyword_token_val_norm = _normalize(last_keyword_token)
			last_last_keyword_token_val_norm = _normalize(last_last_keyword_token) if last_last_keyword_token else last_last_keyword_token_val_norm

			if last_keyword_token_val_norm in table_markers:
				last_table_name = t.value
				last_last_keyword_token = last_keyword_token

			elif last_table_name and last_keyword_token_val_norm in ["AS"]:
				result[t.value] = last_table_name
				last_table_name = False

			elif last_table_name and t.ttype is Name and last_last_keyword_token_val_norm in table_markers:
				#print(last_table_name, t)
				result[t.value] = last_table_name
				last_table_name = False
				last_last_keyword_token = False

		last_keyword_token = t if t.is_keyword else False
